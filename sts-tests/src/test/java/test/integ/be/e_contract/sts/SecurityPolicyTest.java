...
public class SecurityPolicyTest {
	...
	public void setUp() throws Exception {
		...
		this.endpoint2 = Endpoint.publish(this.url2, new ExampleSecurityPolicyServicePortImpl2());

		this.url3 = "https://localhost:" + sslFreePort + "/example/ws3";
		this.endpoint3 = Endpoint.publish(this.url3, new ExampleSecurityPolicyServicePortImpl3());

		this.url4 = "https://localhost:" + sslFreePort + "/example/ws4";
		this.endpoint4 = Endpoint.publish(this.url4, new ExampleSecurityPolicyServicePortImpl4());

		this.url5 = "https://localhost:" + sslFreePort + "/example/ws5";
		this.endpoint5 = Endpoint.publish(this.url5, new ExampleSecurityPolicyServicePortImpl5());

		this.stsUrl = "https://localhost:" + sslFreePort + "/example/sts";
		this.stsEndpoint = Endpoint.publish(this.stsUrl, new ExampleSecurityTokenService());

		this.sts2Url = "https://localhost:" + sslFreePort + "/example/sts2";
		this.sts2Endpoint = Endpoint.publish(this.sts2Url, new ExampleSecurityTokenServiceProvider());

		this.stsUsernameUrl = "https://localhost:" + sslFreePort + "/example/sts-username";
		...
		this.endpoint = Endpoint.publish(this.url, new ExampleSecurityPolicyServicePortImpl());

		this.url6 = "https://localhost:" + sslFreePort + "/example/ws6";
		this.endpoint6 = Endpoint.publish(this.url6, new ExampleSecurityPolicyServicePortImpl6());

		this.url7 = "https://localhost:" + sslFreePort + "/example/ws7";
		this.endpoint7 = Endpoint.publish(this.url7, new ExampleSecurityPolicyServicePortImpl7());

		this.url8 = "https://localhost:" + sslFreePort + "/example/ws8";
		this.endpoint8 = Endpoint.publish(this.url8, new ExampleSecurityPolicyServicePortImpl8());
	...
	public void testPublicKeyToken() throws Exception {
		...
		properties.put(SecurityConstants.STS_TOKEN_CRYPTO, new ClientCrypto(privateKey, certificates));
		stsClient.setProperties(properties);
	...
	public void testPublicKeyTokenOtherKey() throws Exception {
		...
		properties.put(SecurityConstants.SIGNATURE_CRYPTO, new ClientCrypto(privateKey, certificates));
		// next is for holder of key
		properties.put(SecurityConstants.STS_TOKEN_CRYPTO, new ClientCrypto(hokPrivateKey, hokCertificates));
		stsClient.setProperties(properties);
	...
	public void testSupportingTokensUsernameToken() throws Exception {
		...
		assertEquals("username:hello world", result);
	}
	...
	public void testSecureConversationBootstrapUsernameToken() throws Exception {
		...
		BindingProvider bindingProvider = (BindingProvider) port;
		Map<String, Object> requestContext = bindingProvider.getRequestContext();
		requestContext.put(BindingProvider.ENDPOINT_ADDRESS_PROPERTY, this.url6);
	...
	@Test
	public void testTransportBindingHttpsTokenSupportingTokensUsernameToken() throws Exception {
		SpringBusFactory bf = new SpringBusFactory();
		Bus bus = bf.createBus("cxf-https-trust-all.xml");
		BusFactory.setDefaultBus(bus);
		// get the JAX-WS client
		...
		BindingProvider bindingProvider = (BindingProvider) port;
		Map<String, Object> requestContext = bindingProvider.getRequestContext();
		requestContext.put(BindingProvider.ENDPOINT_ADDRESS_PROPERTY, this.url2);
	...
	@Test
	public void testTransportBindingHttpsTokenSupportingTokensSamlToken() throws Exception {
		DocumentBuilderFactory documentBuilderFactory = DocumentBuilderFactory.newInstance();
		documentBuilderFactory.setNamespaceAware(true);
		DocumentBuilder documentBuilder = documentBuilderFactory.newDocumentBuilder();
		Document document = documentBuilder
				.parse(SamlClientCallbackHandler.class.getResourceAsStream("/saml-assertion.xml"));
		...
		BindingProvider bindingProvider = (BindingProvider) port;
		securityDecorator.decorate(bindingProvider, assertionElement, this.url3);
	...
	@Test
	public void testTransportBindingHttpsTokenSupportingTokensX509Token() throws Exception {
		SpringBusFactory bf = new SpringBusFactory();
		Bus bus = bf.createBus("cxf-https-trust-all.xml");
		BusFactory.setDefaultBus(bus);
		// get the JAX-WS client
		...
		BindingProvider bindingProvider = (BindingProvider) port;
		Map<String, Object> requestContext = bindingProvider.getRequestContext();
		requestContext.put(BindingProvider.ENDPOINT_ADDRESS_PROPERTY, this.url4);
		...
		requestContext.put(SecurityConstants.SIGNATURE_CRYPTO,
				new ClientCrypto(privateKey, Collections.singletonList(certificate)));

		// invoke the web service
		String result = port.echo("hello world");
		assertEquals("CN=Test:hello world", result);

		bus.shutdown(true);
	}

	@Test
	public void testTransportBindingHttpsTokenSupportingTokensSamlTokenViaSTS() throws Exception {
		SpringBusFactory bf = new SpringBusFactory();
		Bus bus = bf.createBus("cxf-https-trust-all.xml");
		BusFactory.setDefaultBus(bus);
		// get the JAX-WS client
		...
		BindingProvider bindingProvider = (BindingProvider) port;
		Map<String, Object> requestContext = bindingProvider.getRequestContext();
		requestContext.put(BindingProvider.ENDPOINT_ADDRESS_PROPERTY, this.url5);
		...
		// invoke the web service
		String result = port.echo("hello world");
		assertEquals("custom-CN=Test:hello world", result);

		bus.shutdown(true);
	}

	@Test
	public void testTransportBindingHttpsTokenSupportingTokensSamlTokenViaSTSWithMEX() throws Exception {
		SpringBusFactory bf = new SpringBusFactory();
		Bus bus = bf.createBus("cxf-https-trust-all.xml");
		BusFactory.setDefaultBus(bus);
		// get the JAX-WS client
		...
		BindingProvider bindingProvider = (BindingProvider) port;
		Map<String, Object> requestContext = bindingProvider.getRequestContext();
		requestContext.put(BindingProvider.ENDPOINT_ADDRESS_PROPERTY, this.url7);
	...
	@Test
	public void testTransportBindingHttpsTokenSupportingTokensSamlTokenViaSTSWithMEXAndClaims() throws Exception {
		SpringBusFactory bf = new SpringBusFactory();
		Bus bus = bf.createBus("cxf-https-trust-all.xml");
		BusFactory.setDefaultBus(bus);
		// get the JAX-WS client
		...
		BindingProvider bindingProvider = (BindingProvider) port;
		Map<String, Object> requestContext = bindingProvider.getRequestContext();
		requestContext.put(BindingProvider.ENDPOINT_ADDRESS_PROPERTY, this.url8);
		...
		requestContext.put(SecurityConstants.CALLBACK_HANDLER, new ExampleSecurityPolicyCallbackHandler());
		requestContext.put(SecurityConstants.PREFER_WSMEX_OVER_STS_CLIENT_CONFIG, "true");
		requestContext.put(SecurityConstants.STS_TOKEN_ACT_AS, new ActAsCallbackHandler());
